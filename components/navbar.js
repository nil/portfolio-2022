...
const Navbar = () => {
  ...
  return (
    <nav className='py-[24px] text-s border-b-s border-navbar-border'>
          ...
          {pageLinks.map(({ labelKey, path, pathCheck }, index) => (
            <Link href={path} key={labelKey}>
              <a className={`
                ...
                ${testPath(pathCheck || labelKey) ? 'border-b-xs boder-dashed border-navbar-underline' : ''}
              `}>{navbar[labelKey]}</a>
            </Link>
          ))}
        </div>
        <Link href="/">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className='fill-brand-40 h-[24px] cursor-pointer'>

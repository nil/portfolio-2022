...
const dashBorderPlugin = plugin(({ addUtilities, theme }) => {
  ...
  function generateProperties(pos, widthKey, widthValue, isBigGap) {
    const multiplier = isBigGap ? 2 : 1;
    const direction = pos === 'top' || pos === 'bottom' ? 'right' : 'bottom';
    const dashSize = theme(`borderDashed.${widthKey}`) || 0;
    const dashGap = `${parseInt(theme(`borderGap.${widthKey}`), 10) * multiplier}px` || 0;
  ...
  const borderDashUtilites = positions.map((pos) => (Object.entries(theme('borderWidth')).map(([widthKey, widthValue]) => ({
    [`.border-${pos.charAt(0)}-${widthKey}`]: generateProperties(pos, widthKey, widthValue, false),
    [`.border-${pos.charAt(0)}-${widthKey}.border-big-gap`]: generateProperties(pos, widthKey, widthValue, true),
  }))));
